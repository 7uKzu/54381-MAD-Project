...
export async function register(req, res) {
  ...
  try {
    ...
    const user = await User.create(
      { email, password_hash: hash, full_name, RoleId: roleRow.id, status: role === 'Donor' || role === 'Recipient' ? 'approved' : 'pending' },
      { include: [Role] }
    );

    user.Role = roleRow;
    const { accessToken, refreshToken } = signTokens({ id: user.id, Role: roleRow });
    res.json({ user: await serializeUser(user), accessToken, refreshToken });
  } catch (e) {
    console.error("REGISTER ERROR:", e);  // ← ADD THIS
